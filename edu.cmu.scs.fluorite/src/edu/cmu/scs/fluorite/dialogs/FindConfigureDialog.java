...
public class FindConfigureDialog extends Dialog
{
	...
	public FindConfigureDialog(Shell shell, String initialSearchString, ITextViewer viewer)
	{
		...
		mViewer = viewer;
		mFindReplaceTargetExt = (IFindReplaceTargetExtension)viewer.getFindReplaceTarget();
		mFindReplaceTargetExt3 = (IFindReplaceTargetExtension3)viewer.getFindReplaceTarget();
	...
	protected Control createDialogArea(Composite parent)
	{
		...
		mSearchText.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));
		
		label = new Label(stringComp, SWT.NONE);
		label.setText("R&eplace with:");
		
		mReplaceText = new Text(stringComp, SWT.BORDER);
		mReplaceText.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));
		...
		mAll.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				mViewer.setSelectedRange(mInitialSelection.x, mInitialSelection.y);
			}
		...
		mSelectedLines.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				mFindReplaceTargetExt.setScope(new Region(lineInfo.x, lineInfo.y));
		...
		optionsGroup.setLayoutData(new GridData(SWT.FILL, SWT.TOP, true, false, 2, 1));
		...
		mRegExpMode.setLayoutData(new GridData(SWT.FILL, SWT.TOP, true, false, 2, 1));
		...
		mReplaceAndFindButton = createButton(buttonsComp, REPLACE_AND_FIND, "Replace/Fin&d", false);
		...
		buttonsComp.setLayoutData(new GridData(SWT.RIGHT, SWT.BOTTOM, true, true));
		...
		mCaseSensitive.setSelection(prefStore.getBoolean(Initializer.Pref_FindCaseSensitive));
		mWrap.setSelection(prefStore.getBoolean(Initializer.Pref_FindWrapSearch));
		mMatchWord.setSelection(prefStore.getBoolean(Initializer.Pref_FindWholeWord));
		
		// TODO: implement incremental find later.
		mIncremental.setEnabled(false);
		
		mRegExpMode.setSelection(prefStore.getBoolean(Initializer.Pref_FindRegExp));
		...
		try {
			if (mViewer.getDocument().getLineOfOffset(selectedRange.x) != mViewer.getDocument().getLineOfOffset(selectedRange.x + selectedRange.y)) {
				Point lineInfo = mFindReplaceTargetExt.getLineSelection();
				mFindReplaceTargetExt.setScope(new Region(lineInfo.x, lineInfo.y));
				mViewer.setSelectedRange(lineInfo.x, 0);
		...
		// when the search string is changed, the replace buttons should be disabled.
	...
	private void performReplace() {
		mFindReplaceTargetExt3.replaceSelection(mReplaceText.getText(), mRegExpMode.getSelection());
		setReplaceButtonsEnabled(false);
	}
	...
	private int performFind(int offset) {
		int ret = mFindReplaceTargetExt3.findAndSelect(offset, mSearchText.getText(), mForward.getSelection(),
				mCaseSensitive.getSelection(), mMatchWord.getSelection(), mRegExpMode.getSelection());
		if (ret == -1 && mWrap.getSelection()) {
			ret = mFindReplaceTargetExt3.findAndSelect(0, mSearchText.getText(), mForward.getSelection(),
					mCaseSensitive.getSelection(), mMatchWord.getSelection(), mRegExpMode.getSelection());
		}
	...
	public boolean close() {
		...
		IPreferenceStore prefStore = Activator.getDefault().getPreferenceStore();
		if (prefStore != null) {
			prefStore.setValue(Initializer.Pref_FindForward, mForward.getSelection());
			prefStore.setValue(Initializer.Pref_FindCaseSensitive, mCaseSensitive.getSelection());
			prefStore.setValue(Initializer.Pref_FindWrapSearch, mWrap.getSelection());
			prefStore.setValue(Initializer.Pref_FindWholeWord, mMatchWord.getSelection());
			prefStore.setValue(Initializer.Pref_FindRegExp, mRegExpMode.getSelection());
		}
